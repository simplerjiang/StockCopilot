...
如果不看代码，只从系统结构来理解，`StockCopilot` 大致可以看成 4 层：
...
这个项目比较不一样的一点，是它不想只停在“给个分析结论”，而是尽量把研究结果往后推到动作和复盘：
...
从工程角度看，`StockCopilot` 也不只是“一个前端界面 + 一个后端接口”，它更接近一套完整的股票研究工作台：

- 上层是桌面工作台
- 中层是多 Agent / 多 MCP 编排与业务状态机
- 下层是本地数据、资讯、财报、市场快照、交易记录与模型治理能力

它不是通用的 LangChain 克隆，但在股票场景里，它已经长出了一套**自研的研究编排能力**：
...
很多股票软件擅长“看盘”，很多 AI Demo 擅长“问答”，但两件事真的被打通、还能形成闭环的项目并不多。
...
如果只看 UI，它像一个桌面炒股工作台；但如果看里面的结构，它其实更像一个股票垂直 AI 研究引擎。
