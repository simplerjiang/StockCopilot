...
- [ ] GOAL-012-R3 统一策略注册表与多信号叠加（已启动 Phase A：新增 `chartStrategyRegistry.js` / `chartPanes.js`，把原本散落在 view 层的 legend 升级为统一 strategy registry + grouped chips + render plan。当前已接入并验证首批策略：MA5/10/20/60、VWAP、BOLL、Donchian、MACD、RSI、KDJ、ORB，以及既有 AI 价位线/昨收基线/量能副图；已通过前端单测、build 与后端托管页面 Browser MCP 点击验收。剩余 Phase B/C 将继续补齐 ATR、九转、金叉死叉、突破/假突破、缺口、量价背离与 VWAP 强弱等信号。）
...
- [ ] GOAL-008 交易计划引擎（盘前计划、盘中触发、失效条件）
	...
	- Step 4.3 首版已提交但本轮 reviewer 未通过：虽然 `TradingPlanTriggerService`、`TradingPlanTriggerWorker`、`TradingPlanEvents` 与 `/api/stocks/plans/alerts` 已落地，且后端/前端定向测试可通过，但仍存在 3 个 blocker 需返工后才能算完成：触发引擎尚未真正以 `ActiveWatchlist` 作为执行边界、量价背离 warning 仅按 `MetadataJson` 精确字符串去重导致连续轮询下仍可能重复落同类事件、前端短轮询只稳定覆盖当前股票卡而未把交易计划总览纳入持续刷新。因此 Step 4.3 当前状态应视为“待返工”，尚不能推进 Step 4.4。
	- Dev2 并行前端支线：GOAL-012-R2 已完成 `klinecharts` 受控替换；GOAL-012-R3 现已进入 Phase A 并完成首批策略注册表落地，在既有 `charting/**` 适配层上实现统一 strategy registry、grouped chips 与 render plan，当前已支持 MA/VWAP/BOLL/Donchian、MACD/KDJ/RSI、ORB 及既有 AI/昨收/量能图层。后续仍将继续补齐 ATR、神奇九转、各类金叉死叉、突破/假突破、缺口、量价背离与 VWAP 强弱，并保持按钮化切换与父层 contract 稳定。
