...
2. Dev1 当前返工主线：Step 4.3 盘中定量规则盯盘引擎已提交首版，但未通过本轮 reviewer 验收，必须先消除 Step 4.3 blocker 后才允许推进 Step 4.4。
...
状态标签：`待返工（review blocker）`

### 当前 reviewer 结论
Step 4.3 首版实现已经具备基础框架、测试和前端展示，但本轮提测不通过，原因不是“没代码”，而是存在 3 个会影响任务定义成立的 blocker。Dev1 必须先把这 3 个问题修完并重测，才能把 Step 4.3 记为完成。

### 必修 blocker
1. 触发引擎必须接回 `ActiveWatchlist` 作为执行边界：
   - 当前实现直接扫描全部 `Pending` 计划，没有把 `ActiveWatchlist` 当作白名单门控。
   - 返工后必须保证 Step 4.3 只对“已进入 `ActiveWatchlist` 且本地缓存可用”的计划执行判断，不能绕开 Step 4.1 底座直接全表扫计划。
2. 量价背离 warning 的幂等去重不合格：
   - 当前只按 `MetadataJson` 全量字符串相等做去重，分钟数据窗口轻微变化时，连续轮询仍可能反复写入同类 warning。
   - 返工后必须改成“按计划 + 事件类型 + 有效时间窗/状态”去重，确保同一段持续背离不会在每轮轮询都重复落事件。
3. 前端短轮询只覆盖当前股票卡，未稳定覆盖交易计划总览：
   - 当前总览卡主要靠首屏加载和手工刷新，不能持续反映最新事件摘要。
   - 返工后必须让“当前计划区 + 交易计划总览”都能通过短轮询看到最新状态/告警，而不是只刷新当前股票。
...
5. 至少完成后端定向单测、前端定向单测、frontend build，以及一次后端托管页面的 Browser MCP 告警/状态刷新验证。
