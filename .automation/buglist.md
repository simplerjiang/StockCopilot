...
### v0.6.0 用户验收发现（2026-04-29）

| Bug# | 严重度 | 描述 | 状态 |
|------|--------|------|------|
| **#115** | P1 | `POST /api/stocks/history` 返回 500，页面显示"历史记录保存失败"。每次切换股票都触发，最近查询持久化失败。 | FIXED |
| **#116** | P1 | `GET /api/news?level=sector&symbol=sh600519` 返回 500，板块级新闻完全不可用。 | FIXED |
| **#117** | P1 | 盘中消息显示"消息降级"+"盘中消息读取超时，已降级返回空列表"，交易时间段内盘中消息完全空白。 | FIXED |
| **#118** | P2 | 导航栏标签在不同 Tab 间显示简称/全称不一致（如"股票"vs"股票信息"）。 | OPEN |
| **#119** | P2 | AI 回测仪表板：1 条样本显示 100.0% 准确率，无小样本警告，可能误导决策。 | FIXED |
| **#120** | P2 | AI 回测明细：同股同日出现两行记录（一行"已计算"一行"数据不足"），缺乏解释。 | OPEN |
| **#121** | P2 | AI 回测记分卡缺少准确率计算规则的说明。 | FIXED |
| **#122** | P3 | AI 回测页面下半部分大量空白，数据少时缺乏引导。 | OPEN |
| **#123** | P3 | 批量回测按钮"处理 0 条"缺乏说明为什么没有可处理数据。 | FIXED |
| **#124** | P3 | 回测明细股票筛选下拉只显示代码，不显示名称。 | FIXED |
