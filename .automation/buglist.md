...
- "股票信息"页面中，选定了一只股票，等待加载的时候没有loading组件提示，有一两秒的无反馈空窗期。
- "股票信息"页面中"AI分析"功能，如果分析过程中，切换了页面，则下次进来会一直卡在分析中，无法输入新的对话。对话框要允许用户打断分析，用户新加的提示词要叠加未完成的分析一起分析
- "公司概况工具","个股新闻工具" 一直请求失败，另外提示“ 外部搜索未启用，StockSearchMcp 当前只返回空结果。请在 LLM 设置中配置 Tavily API Key。” ，我原本已经在default渠道设置了Tavily API Key，但是看起来现在是Tavily API跟随default 渠道，不应该这样，Tavily API是独立的，选用任何LLM渠道都应该使用同一个Tavily API key，修改"LLM设置"页面逻辑和后端逻辑。
- "股票信息"页面中"AI分析"功能，点击正上方的刷新按钮，提示”Failed to execute 'json' on 'Response': Unexpected end of JSON input“
- "情绪轮动" 页面中，很多内容都没有刷新，很明显卡在了4月2号，但是今天已经是4月4号了，4月3号盘中时候也刷新过，但是没有反应，深度测试"情绪轮动"页面，检查背后获取逻辑是否正确，并且要补充loading进度
- "全量资讯库" 页面中，要加上 手动调用 LLM清洗功能，允许用户点击后批量清洗“待清洗”的功能。
- “交易日志” 页面中，我不确定哪些指示（例如过度交易、红色banner，风险塘口）是来自LLM指示，取决原因是什么，值不值得可信，PM需要研究下如何深化这个页面，给出类似弹窗或者hover tip告知用户"由于什么什么原因，做出此决定"。
- 整个系统的提示弹窗，confirm，都不要用html系统自带的alert和confirm，很丑不好看，要用同一UI/UX风格的组件库。