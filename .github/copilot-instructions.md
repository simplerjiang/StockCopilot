...
- Work through accepted scope systematically, keep communication concise, follow sound engineering practice, and keep code as concise and elegant as possible while avoiding redundant or unnecessary code. Keep single files reasonably sized, target under 1000 lines per file whenever practical, and prefer splitting long files into focused components, modules, or imports instead of continuing to expand one file. Only exceed that limit when the file cannot be split cleanly.
