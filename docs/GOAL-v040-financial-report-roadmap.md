...
1. **v0.4.0：财报中心基础落地**
   - 正式页面化
  - 采集结果透明化（从“只报数量”升级为“展示报告期 / 标题 / 来源 / 降级 / PDF 补充摘要”）
  - 本地财报数据表格化（形成正式分页列表、筛选、排序、详情入口，而不是继续停留在单股票摘要视图）
...
| 版本 | 核心目标 | 关键词 | 交付物 |
|---|---|---|---|
| `v0.4.0` | 正式页面化、采集结果透明化、本地财报表格化 | 财报中心、列表、采集摘要、筛选 | 正式页面、分页查询接口、采集内容摘要、报表列表与详情入口 |
...
1. 财报中心页面
2. 采集结果透明化
  - 采集完成后展示报告期、标题摘要、来源渠道、降级说明、PDF 补充摘要
  - 让用户能判断“拿到了什么”，而不是只看到数量
3. 本地财报数据表格化
  - 把已获取财报组织为正式分页列表
  - 支持筛选、排序、详情入口、重新采集入口
4. 正式查询接口
